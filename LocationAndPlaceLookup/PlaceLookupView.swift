...
struct PlaceLookupView: View {
    let locationMananger: LocationManager  //  Passed in from the parent View
    @Binding var selectedPlace: Place?  //  Passed from the parent view
    ...
    var body: some View {
        ...
        .onAppear{  //  Only need to get searchRegion when View appears
            searchRegion = locationMananger.getRegionAroundCurrentLocation() ?? MKCoordinateRegion()
        }
        .onDisappear{
        ...
        .onChange(of: searchText){oldValue, newValue in
            ...
            searchTask = Task{
                do {
                    //  Wait 300ms before running the current Task.  Any typing before the Task has run cancels the old task.  This prevents searches happening quickly if a user types fast and will reduce chances that Apple cuts off search becuase too many searches execute too quickly.
...
#Preview {
    PlaceLookupView(locationMananger: LocationManager(), selectedPlace: .constant(Place(mapItem: MKMapItem())))
}
